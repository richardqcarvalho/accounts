...
export function calcIRRF(proLabore) {
  const base = Math.max(0, proLabore - calcINSS(proLabore))
  const bracket = IRRF.brackets.find((b) => base <= b.upTo)
...
export function rbt12ForMonth(calIndex, currentRevenue, revenueByCalIndex) {
  let sum = 0
...
function truncCents(reais) {
  return Math.floor(reais * 100) / 100
}

function faixaFor(rbt12) {
  return (
...
function effectiveRate(rbt12, faixa) {
...
export function calcSimples({ internal, external, rbt12Internal, rbt12External }) {
  let internalRate = 0
  let dasInternal = 0
  if (internal > 0) {
